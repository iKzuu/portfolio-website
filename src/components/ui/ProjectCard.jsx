  ...
  return (
    <div className="relative flex-1 flex flex-col p-2 xl:p-4 gap-2 xl:gap-4 bg-light border-4 border-black shadow-accent-lg-hard">
      ...
      <div className="relative flex flex-col flex-1">
        ...
        <h1 className={`${pixel.className} text-2xl lg:text-4xl 2xl:text-6xl font-semibold text-dark`}>{name}</h1>

        <p className={`${pixel.className} text-sm md:text-base xl:text-2xl text-neutral`}>{description}</p>

        <Link href={href} target="_blank" className="absolute top-0 right-0 flex items-center p-1 xl:p-2 w-fit bg-accent border-2 border-black shadow-black-sm-hard">
          <LinkIcon color="black" className="w-4 h-4 md:w-4 md:h-4 xl:w-5 xl:h-5 2xl:w-8 2xl:h-8"/>
        </Link>

        <div className="flex flex-wrap items-center gap-2 lg:gap-4 mt-auto pt-4">
          {tech.map((item, index) => (
            <span key={index} className="bg-neutral w-fit p-2 text-xs lg:text-base font-semibold">{item}</span>
          ))}
        </div>
